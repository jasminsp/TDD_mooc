...
function createApp(database) {
  ...
  app.get("/prices", (req, res) => {
    ...
    const cost = calculateCost(age, type, baseCost, date, date2);
    res.json({ cost });
  });
  ...
  function calculateCost(age, type, baseCost, date, date2) {
    if (type === "night") {
      return calculateCostForNightTicket(age, baseCost);
    } else {
      return calculateCostForDayTicket(age, baseCost, date, date2);
    }
  }
  ...
  function calculateCostForDayTicket(age, baseCost, date, date2) {
  ...
  function isHoliday(date2, date) {
    ...
    for (let row of holidays) {
      ...
      if (
        ...
        date2.monthCode === holiday.monthCode &&
